...
const Attendance = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-base)] p-4 md:p-10 animate-in fade-in duration-700 transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 md:mb-12">
        <div>
          <h1 className="text-3xl md:text-5xl font-black flex items-center gap-4 tracking-tighter uppercase italic text-[var(--text-primary)]">
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <p className="text-[var(--text-secondary)] text-[10px] font-mono uppercase tracking-[0.2em]">
        ...
        <button 
          onClick={fetchLogs}
          disabled={loading}
          className="flex items-center justify-center gap-3 bg-[var(--surface-input)] hover:bg-cyan-500/10 border border-[var(--border-default)] hover:border-cyan-500/30 px-8 py-4 rounded-2xl transition-all text-[10px] font-black uppercase tracking-[0.2em] disabled:opacity-50 group text-[var(--text-primary)]"
        >
      ...
      {/* 3. TABLE SECTION */}
      <div className="border border-[var(--border-subtle)] bg-[var(--surface-card)] rounded-[35px] md:rounded-[45px] overflow-hidden backdrop-blur-3xl shadow-2xl transition-colors duration-300">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-[var(--surface-elevated)] text-[var(--text-secondary)] text-[10px] uppercase tracking-[0.3em] font-black">
              <tr>
            ...
            <tbody className="text-sm">
              {!loading && logs.length > 0 ? logs.map((log) => (
                <tr key={log.id} className="border-b border-[var(--border-subtle)] hover:bg-[var(--surface-hover)] transition-all duration-300 group">
                  <td className="p-8">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <div className="font-bold text-[var(--text-primary)] text-base tracking-tight">{log.studentName}</div>
                  ...
                  <td className="p-8">
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2 text-[var(--text-primary)] font-bold uppercase text-xs">
                        <LayoutGrid size={14} className="text-cyan-500" /> {log.groupName}
                      </div>
                      <div className="text-[10px] text-[var(--text-secondary)] font-mono tracking-wider">{log.subjectName}</div>
                    </div>
                  </td>
                  <td className="p-8 text-center">
                    <div className="font-mono text-[var(--text-primary)] font-bold text-base">
                      {new Date(log.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                    </div>
                    <div className="text-[9px] text-[var(--text-muted)] uppercase mt-1">{new Date(log.createdAt).toLocaleDateString()}</div>
                  </td>
                    ...
                    <div className="flex flex-col items-center gap-6">
                      {loading ? (
                        ...
                        <div className="w-16 h-16 bg-[var(--surface-hover)] rounded-full flex items-center justify-center">
                          <UserCheck className="text-[var(--text-muted)]" size={32} />
                        </div>
                      )}
                      <div className="space-y-2">
                        <p className="text-[var(--text-primary)] font-black uppercase tracking-widest text-sm">
                          {loading ? "Scanning Database..." : error ? "Connection Failed" : "No Activity Detected"}
                        </p>
                        <p className="text-[var(--text-muted)] text-[10px] font-mono uppercase max-w-xs mx-auto">
      ...
      {/* 4. FOOTER */}
      <div className="mt-10 flex justify-between items-center text-[var(--text-muted)] text-[9px] font-mono uppercase tracking-[0.3em]">
